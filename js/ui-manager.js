...
export class UIManager {
  ...
  renderSequenceGrid(contact) {
    ...
    // Add accepted field to the sequence steps for display
    const allSteps = [
      "warmup",
      "connect",
      "accepted",
      ...CONFIG.SEQUENCE_STEPS.filter((s) => s !== "warmup" && s !== "connect"),
    ]
    ...
    allSteps.forEach((step) => {
      ...
      // Make accepted field read-only (no checkbox)
      if (step === "accepted") {
        item.innerHTML = `
          <div style="width: 14px; height: 14px; margin-top: 1px; display: flex; align-items: center; justify-content: center; font-size: 10px;">
            ${isCompleted ? "✅" : "⏳"}
          </div>
          <div style="flex: 1;">
            <div class="sequence-label">${step} ${isCompleted ? "(Auto)" : ""}</div>
